...
for c in [
    'Email-рассылка и автоматические письма',
    'База клиентов — все контакты автоматически попадают в CRM',
    'Регистрация участниц',
    'Регистрация спонсоров',
...
for c in [
    ...
    'Спонсорские пакеты: Gold, Platinum, Титульный',
    'Форма «Become a Sponsor»',
    'Возможность сразу оплатить пакет или оставить заявку',
]: item(c)
...
h2('Для бизнеса')
for c in [
    'CRM — все заявки автоматически попадают в базу',
    'Автоматические email-цепочки: подтверждение, напоминания, инструкции, благодарность',
    'Промокоды',
    'Партнёрская программа (affiliate-ссылки)',
    'Голосование «People\'s Choice» с защитой от накрутки',
    'Кабинет организатора — управление участницами, рассылки, статусы',
]: item(c)

h2('Для SEO')
for c in [
    'Оптимизированные страницы под поиск Google',
    'Блог с 4 стартовыми статьями:',
]: item(c)
for article in [
    ...
    p = doc.add_paragraph()
    m = p.add_run('     —  '); f(m, 10.5, color=LIGHT)
    t = p.add_run(article); f(t, 10.5, color=MID, italic=True)
    p.paragraph_format.left_indent = Inches(0.4)
    p.paragraph_format.space_after = Pt(2)

h2('Доверие и юридика')
for c in [
    'FAQ',
    'Контакты',
...
doc.save('/sessions/relaxed-jolly-pasteur/mnt/outputs/CoreNova_MissMiami_Стоимость_v3.docx')
print('Done!')
